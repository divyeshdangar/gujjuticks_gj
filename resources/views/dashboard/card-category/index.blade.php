    ...
    <div class="card bg-white border-0 rounded-10 mb-4">
        <div class="card-body p-4">
            <div class="d-sm-flex text-center justify-content-between align-items-center border-bottom pb-20 mb-20">
                <h4 class="fw-semibold fs-18 mb-sm-0">{{ __('dashboard.card_category') }}</h4>
                <a href="{{ route('dashboard.card.category.create') }}"
                    class="border-0 btn btn-primary py-2 px-4 text-white fs-14 fw-semibold rounded-3">
                    <span class="py-sm-1 d-block">
                        <i class="ri-add-line text-white"></i>
                        <span>{{ __('dashboard.add') }} {{ __('dashboard.card_category') }}</span>
                    </span>
                </a>
            </div>
                    ...
                    <table class="table align-middle">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">{{ __('dashboard.cards') }}</th>
                        ...
                        <tbody>
                            @foreach ($dataList as $data)
                                <tr>
                                    ...
                                    <td>
                                        <a target="_blank"
                                            href="{{ URL::asset('/images/card-category/' . $data->image) }}">
                                            <img src="{{ URL::asset('/images/card-category/' . $data->image) }}"
                                    ...
                                    <td>
                                        ...
                                        <div class="dropdown action-opt">
                                            @if ($data->deleted_at == null)
                                                <a class="btn bg p-1"
                                                    href="{{ route('dashboard.card.category.edit', ['id' => $data->id]) }}">
                                                    <i data-feather="edit-3"></i>
                                                </a>
                                                <a class="btn bg p-1"
                                                    onclick="confirmAndDelete('{{ route('dashboard.card.category.delete', ['id' => $data->id]) }}')">
                                                    <i data-feather="trash-2"></i>
                                                </a>
                                            @else
                                                <a title="Restore" class="btn bg p-1"
                                                    onclick="confirmAndDelete('{{ route('dashboard.card.category.restore', ['id' => $data->id]) }}', 'restore')">
