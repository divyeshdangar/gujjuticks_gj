    ...
    <div class="card bg-white border-0 rounded-10 mb-4">
        <div class="card-body p-4">
            <div class="d-sm-flex text-center justify-content-between align-items-center border-bottom pb-20 mb-20">
                <h4 class="fw-semibold fs-18 mb-sm-0">{{ __('dashboard.cards') }}</h4>
                <a href="{{ route('dashboard.card.create') }}"
                    ...
                    <span class="py-sm-1 d-block">
                        ...
                        <span>{{ __('dashboard.add') }} {{ __('dashboard.card') }}</span>
                    </span>
                </a>
            </div>
                        ...
                        <tbody>
                            @foreach ($dataList as $data)
                                <tr>
                                    ...
                                    <td width="300px">
                                        <a target="_blank" href="{{ URL::asset('/images/card/' . $data->front_image) }}">
                                            <img src="{{ URL::asset('/images/card/' . $data->front_image) }}" width="120px" class="img-thumbnail">
                                        </a>
                                        <a target="_blank" href="{{ URL::asset('/images/card/' . $data->back_image) }}">
                                            <img src="{{ URL::asset('/images/card/' . $data->back_image) }}" width="120px" class="img-thumbnail">
                                        </a>
                                    </td>
                                    <td class="ps-0">
                                    ...
                                    <td>
                                        ...
                                        <div class="dropdown action-opt">
                                            @if ($data->deleted_at == null)
                                                <a class="btn bg p-1"
                                                    href="{{ route('dashboard.card.edit', ['id' => $data->id]) }}">
                                                    <i data-feather="edit-3"></i>
                                                </a>
                                                <a class="btn bg p-1"
                                                    onclick="confirmAndDelete('{{ route('dashboard.card.delete', ['id' => $data->id]) }}')">
                                                    <i data-feather="trash-2"></i>
                                                </a>
                                            @else
                                                <a title="Restore" class="btn bg p-1"
                                                    onclick="confirmAndDelete('{{ route('dashboard.card.restore', ['id' => $data->id]) }}', 'restore')">
