        ...
        <div class="col-lg-8">
            <div class="card bg-white border-0 rounded-10 mb-4">
                <div class="card-body p-4">
                    <h4 class="fw-semibold fs-18 border-bottom pb-20 mb-20">{{ __('dashboard.blog') }} {{ __('dashboard.edit') }}</h4>
                    @if ($errors->any())
                    ...
                    <form method="post" id="formToValidate"
                        action="{{ route('dashboard.blog.category.edit.post', ['id' => $dataDetail->id]) }}">
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <div class="form-group mb-4">
                                    <label class="label @error('parent_id') text-danger @enderror">{{ __('dashboard.blog_category') }}</label>
                                    <div class="form-group position-relative">
                                        <select class="form-select form-control ps-5 h-58" name="parent_id" aria-label="Parent category selection">
                                            <option class="text-dark">{{ __('dashboard.select') }} {{ __('dashboard.blog_category') }}</option>
                                            @foreach ($categoryData as $data)
                                                <option value="{{ $data->id }}" class="text-dark" @if ($data->id == old('parent_id', $dataDetail->parent_id)) selected="selected" @endif>{{ $data->title }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <div class="form-group position-relative">
                                        <textarea id="description" name="description"
                                            ...
                                            placeholder="{{ __('dashboard.description') }}" required>{{ old('description', $dataDetail->description) }}</textarea>
                                    </div>
                                    @error('description')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
        ...
        <div class="col-lg-4">
            <div class="card bg-white border-0 rounded-10 mb-4">
                <div class="card-body p-4">
                    ...
                    <img src="{{ URL::asset('/images/blog-category/'.$dataDetail->image) }}" class="img-thumbnail">
                </div>
            </div>
        </div>
    </div>
