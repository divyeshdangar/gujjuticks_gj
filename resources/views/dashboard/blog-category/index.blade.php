    ...
    <div class="card bg-white border-0 rounded-10 mb-4">
        <div class="card-body p-4">
            <div class="d-sm-flex text-center justify-content-between align-items-center border-bottom pb-20 mb-20">
                <h4 class="fw-semibold fs-18 mb-sm-0">{{ __('dashboard.blog_category') }}</h4>
                <a href="{{ route('dashboard.blog.category.create') }}" class="border-0 btn btn-primary py-2 px-4 text-white fs-14 fw-semibold rounded-3">
                    <span class="py-sm-1 d-block">
                        <i class="ri-add-line text-white"></i>
                        <span>{{ __('dashboard.add') }} {{ __('dashboard.blog_category') }}</span>
                    </span>
                </a>
            </div>
                        ...
                        <tbody>
                            @foreach ($dataList as $data)
                                <tr>
                                    ...
                                    <td>
                                        <a target="_blank" href="{{ URL::asset('/images/blog-category/'.$data->image) }}">
                                            <img src="{{ URL::asset('/images/blog-category/'.$data->image) }}" width="50px" class="img-thumbnail">
                                        </a>
                                    </td>
                                    <td class="ps-0">
                                    ...
                                    <td>
                                        <div class="dropdown action-opt">
                                            <a class="btn bg p-1" href="{{ route('dashboard.blog.category.view', ['id' => $data->id]) }}">
                                                <i data-feather="eye"></i>
                                            </a>
                                            <a class="btn bg p-1" href="{{ route('dashboard.blog.category.edit', ['id' => $data->id]) }}">
                                                <i data-feather="edit-3"></i>
                                            </a>
                                            <a class="btn bg p-1" onclick="confirmAndDelete('{{ route('dashboard.blog.category.delete', ['id' => $data->id]) }}')">
